...
검증기는 외부 네트워크에서 `$id`를 조회하지 않고 다섯 개의 로컬 스키마를 URN 기준으로 레지스트리에 등록한다. 유효·무효 구조뿐 아니라 예제 레코드의 JCS 콘텐츠 해시와 핸드오프가 참조한 체크포인트 해시도 확인한다. 이 Python 명령은 Go 구현 전의 규격 검증 도구이며 `ctx` 실행 환경의 의존성이 아니다. Go 검증 테스트가 같은 사례를 모두 다루면 제거한다.
