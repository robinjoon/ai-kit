...
3. 에이전트가 의미 있는 진행 뒤 `ctx checkpoint`를 직접 호출한다.
4. `ctx resume`은 최신 체크포인트 전체와 현재 Git 차이를 반환한다.
5. `ctx status`는 현재 작업과 마지막 저장 시점을 보여 준다.

체크포인트에는 목표, 현재 요약, 결정(무엇을 왜), 다음 행동과 차단 요소만
저장한다. 각 체크포인트는 이전 기록을 수정하지 않는 독립 JSON 파일이며,
현재 작업은 가장 최근 체크포인트 하나만 가리킨다.
...
에이전트가 직접 `ctx checkpoint`를 호출한다. 다음 시점에 사용자의 별도 저장
요청 없이 실행한다.
...
```json
{
  ...
  "decisions": [
    {
      "what": "만료 쿠키는 즉시 폐기한다.",
      "why": "지연 폐기 시 다른 요청이 만료 세션을 재사용할 위험이 있다."
    }
  ],
...
`goal`과 `summary`만 필수다. `decisions` 배열의 각 항목은 `what`(필수)과
`why`(권장) 두 문자열 필드를 갖는다. CLI는 알 수 없는 필드를 거부하며
JSON Schema나 외부 검증 라이브러리를 사용하지 않는다.
