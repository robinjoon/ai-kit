...
- Set `work_status` to exactly one of `active`, `blocked`, `completed`, or
  `abandoned`.
- Review every semantic section and produce a complete, self-contained current
  state. Handoff does not accept partial capture.
...
- If any section cannot be reviewed, stop and explain what is missing. Do not
  fabricate it, mark the capture complete, or create a draft in place of handoff.
- `blocked` requires an active blocker. `completed` must have no current work or
  active blocker.
- An `active` or `blocked` handoff requires at least one next action.
