...
> 이 문서는 현재 v1 스키마와 합의된 MVP 범위로부터 도출한 **목표 사용 흐름**을 설명한다. 현재 저장소에는 v1 데이터 계약과 검증 도구가 있으며, 아래 CLI와 앱 스킬이 모두 구현됐다는 뜻은 아니다.
...
지원되는 앱 훅이나 다음 ctx 명령 실행 시점에 저장된 런타임 스냅숏은 마지막으로 관측한 기계 상태를 알려 준다. 그러나 의미 체크포인트를 대신하지는 않는다.
...
sequenceDiagram
    ...
    App->>Ctx: 지원되는 수명 주기 훅 또는 ctx 명령
